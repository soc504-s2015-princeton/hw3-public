...
(@) Run a regression model to summarize the relationship between education, income, and occupational prestige?   Here education is the percentage of people in the field who have graduated from high school in 1950.

```{r}
# [your code here]
```

(@) Now run a regression model where education is used to predict occupational prestige and the relationship between education and occupational prestige is allowed to vary by occupation type (e.g., white collar, blue collar).

```{r}
# [your code here]
```

(@)  In words, how would you summarize the conclusions from the model above?

```{answer}
your answer here
```

(@) Make a scatter plot showing the relationship between education and prestige for blue collar and white collar workers.  Include the best fit lines for each group.
